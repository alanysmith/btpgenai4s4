 ...
 * @Before(event = { "READ" }, entity = "alansmith_34_a42Srv.CustomerMessage")
...
try {
    ...
    const customerMessages = await SELECT.from('alansmith_34_a42.CustomerMessage').forUpdate();
    ...
    await Promise.all(customerMessages.map(async customerMessage => {
        ...
        if (!titleEnglish || !messageCategory || !messageUrgency || !messageSentiment || !summaryCustomerLanguage || !summaryEnglish || !fullMessageEnglish) {
            ...
            await UPDATE('alansmith_34_a42.CustomerMessage')
