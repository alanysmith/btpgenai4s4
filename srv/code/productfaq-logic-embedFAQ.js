/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "alansmith_34_a42Srv.ProductFAQ")
...
try {
	...
	let productFAQ = await SELECT.one("alansmith_34_a42.ProductFAQ").where({ ID: productFAQID }).forUpdate();
	...
	await UPDATE("alansmith_34_a42.ProductFAQ", ID).with({ embedding: utils.array2VectorBuffer(embedding) });
